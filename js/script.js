...
var coverArray = new Array('<img src="images/cover.jpg">', '<img src="images/coverTwo.jpg">', '<img src="images/coverThree.png">', '<img src="images/coverFour.png">', '<img src="images/coverFive.png">', '<img src="images/coverSix.png">', '<img src="images/coverSeven.png">', '<img src="images/coverEight.png">', '<img src="images/coverNine.png">');

//loads the definition of wordOne on the DOM ***THIS WAS WHERE I SPENT 99% OF MY TIME, TRYING TO MAKE THE RESULTS DISPLAY CORRECTLYS***
var loadDef = function(){
  //if there is an error, stop the function
  if(responseOne.error){
    return;
  };
  //specifies the part of the definition info we're looking for
  var definition = responseOne.text;
  console.log(definition);
  //displays definition in hovercard that displays when you hover over the wordOne div
  $('#defOne').html('<p><em>Definition:</em> ' + definition + '</p>');
  $('#wordOne').hovercard({
    detailsHTML:$('#defOne').html(),
    width: 200,
    autoAdjust: false
  });

  // failed attempt to pick out part of the result data from the array of results returned by the API
  //I struggled SO HARD with this part. 
  // for(i = 0; i < 2; i++){
  //   return response[i];
...
var loadDefTwo = function(){
  //if there is an error, stop the function
  if(responseTwo.error){
    return;
  };

  var definitionTwo = responseTwo.text;
  console.log(definitionTwo);
  //displays definitionTwo in a hovercard that displays when you hover over the wordTwo div
  $('#defTwo').html('<p><em>Definition:</em> ' + definitionTwo + '</p>');
  $('#wordTwo').hovercard({
    detailsHTML:$('#defTwo').html(),
    width: 200,
    autoAdjust: false
  });
};
...
var getDef = function(response) {
  ...
  $.ajax({
    ...
    success : function(response){
      //responseOne is the data from the first slot in the response array—since I'm only looking for one definiton, the array returned only HAS one slot
      //but it was being SUPER ANNOYING about displaying the results until I specified this. 
      responseOne = response[0];
      console.log(responseOne);
      //calls the loadDef function to load the response into the DOM; pass responseOne into the function
      loadDef(responseOne);
    }
...
var getDefTwo = function(response) {
  ...
  $.ajax({
    ...
    success : function(response){
      ...
      //calls the loadDef function to load the response into the DOM; pass responseTwo into the function
      loadDefTwo(responseTwo);
    }
...
var loadWord = function(response){
  ...
  //uses Chance.js library to capitalize the first letter of the returned word
  $('#wordOne').text(chance.capitalize(wordOne));
...
var loadWordTwo = function(response){
  ...
  //uses Chance.js library to capitalize the first letter of the returned word
  $('#wordTwo').text(chance.capitalize(wordTwo));
};
...
//use Chance.js library to pick random cover from cover array
  var chosenCover = chance.pick(coverArray);
  
  loadCover(chosenCover);

  if(chosenCover === coverArray[6]){
    var title = document.getElementById('#wordOne');
    var titleTwo = document.getElementById('#wordTwo');
    $title.addClass('black');
    $titleTwo.addClass('black');
  }

  console.log(chosenCover);
...
//OLD/FAILED ATTEMPTS BELOW (I tried to use the Swagger.js library 
  //for my API calls, but it would not install correctly so I decided to do
  //things the old fashioned way after all)
...
// var getDef = function(response) { 
//   wordOne = response.word;
//   wordnik = new SwaggerApi("http://api.wordnik.com/v4",
//     {
//     api_key: APIKey, // my wordnik API key
//     verbose: true,
//     success: function() {
//       args = {word: wordOne}
